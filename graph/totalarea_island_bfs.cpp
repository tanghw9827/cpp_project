...
int main(){
    ...
    for(int i = 0; i < n; i++){
        if(grid[i][0] == 1 && !visited[i][0]){
            bfs(grid, i, 0, visited, directions, dummy_count);
        }
        if(grid[i][m - 1] == 1 && !visited[i][m - 1]){
            bfs(grid, i, m - 1, visited, directions, dummy_count);
        }
    }
    for(int j = 0; j < m; j++){
        if(grid[0][j] == 1 && !visited[0][j]){
            bfs(grid, 0, j, visited, directions, dummy_count);
        }
        if(grid[n - 1][j] == 1 && !visited[n - 1][j]){
            bfs(grid, n - 1, j, visited, directions, dummy_count);
        }
    }
    int count;
    int result = 0;
